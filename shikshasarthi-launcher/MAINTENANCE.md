...
2.  **Update Hub Manifest**:
    On your **College Hub Server (100.111.94.52:8081)**, update the version in the manifest API:
    ```js
...
### How it works:
- **Hourly Cron**: Every hour, the `sync-agent` checks for a connection to `http://100.111.94.52:8081`.
